...
This was done on 2022-01-19 11:45:33.629000.
*/
...
void isr_ieee154e_inhibitStart(OpenMote* self, opentimers_id_t id);
...
void ieee154e_init(OpenMote* self) {
 ...
 radio_setEndFrameCb(self, ieee154e_endOfFrame);
    ...
    (self->ieee154e_vars).timerId = opentimers_create(self, TIMER_TSCH, TASKPRIO_NONE);
 ...
 IEEE802154_security_init(self);
    (self->ieee154e_vars).serialInhibitTimerId = opentimers_create(self, TIMER_INHIBIT, TASKPRIO_NONE);
}
...
void isr_ieee154e_newSlot(OpenMote* self, opentimers_id_t id) {
    ...
    (self->ieee154e_vars).startOfSlotReference = opentimers_getCurrentCompareValue(self);
 opentimers_scheduleAbsolute(self, 
        (self->ieee154e_vars).timerId,                  // timerId
...
void isr_ieee154e_inhibitStart(OpenMote* self, opentimers_id_t id) {
 ...
 openserial_inhibitStart(self); // activity_inhibitSerial
}
...
port_INLINE void activity_synchronize_newSlot(OpenMote* self) {
 ...
 radio_rxNow(self);
        ...
        if ((self->ieee154e_vars).asn.bytes0and1%(NUM_CHANNELS*EB_PORTION)==0) {
            // turn off the radio (in case it wasn't yet)
 radio_rfOff(self);
 ...
 opentimers_scheduleAbsolute(self, 
        (self->ieee154e_vars).serialInhibitTimerId,     // timerId
...
port_INLINE void activity_synchronize_startOfFrame(OpenMote* self, PORT_TIMER_WIDTH capturedTime) {
    ...
    // inhibit serial activity
 openserial_inhibitStart(self); // synchronize start of frame
...
port_INLINE void activity_ti1ORri1(OpenMote* self) {
 ...
 schedule_getNeighbor(self, &neighbor);
            ...
            if ( schedule_getOkToSend(self)) {
                if ( packetfunctions_isBroadcastMulticast(self, &neighbor)==FALSE){
                    ...
                    if ( schedule_getShared(self)){
                        // this is an autonomous TxRx cell (unicast)
                        (self->ieee154e_vars).dataToSend = openqueue_macGet6PandJoinPacket(self, &neighbor);
                    } else {
                        // this is a managed Tx cell
                        (self->ieee154e_vars).dataToSend = openqueue_macGetNonJoinIPv6Packet(self, &neighbor);

                        if ((self->ieee154e_vars).dataToSend == NULL){
                            (self->ieee154e_vars).dataToSend = openqueue_macGetKaPacket(self, &neighbor);
                        }

                        // update numcellpassed and numcellused on managed Tx cell
                        if ((self->ieee154e_vars).dataToSend!=NULL) {
                            (self->ieee154e_vars).dataToSend->l2_sendOnTxCell = TRUE;
 ...
 msf_updateCellsPassed(self, &neighbor);
                    ...
                    if ( schedule_getShared(self)) {
                        ...
                        if ( msf_getHashCollisionFlag(self)==TRUE){
                            // check whether there is 6p or join request packet to send first
                            (self->ieee154e_vars).dataToSend = openqueue_macGet6PandJoinPacket(self, &neighbor);
                        }

                        if ((self->ieee154e_vars).dataToSend == NULL) {
                            if ( schedule_getAutonomousTxRxCellUnicastNeighbor(self, &autonomousUnicastNeighbor)==TRUE){
                                (self->ieee154e_vars).dataToSend = openqueue_macGet6PRequestOnAnycast(self, &autonomousUnicastNeighbor);
                            }
                        }

                        if ((self->ieee154e_vars).dataToSend == NULL) {
                            (self->ieee154e_vars).dataToSend = openqueue_macGet6PResponseAndDownStreamPacket(self, &neighbor);
                        }
                    }
                }
            }
 ...
 ieee154e_getAsn(self, asn);
            ...
            sctimer_scheduleActionIn(ACTION_LOAD_PACKET, (self->ieee154e_vars).startOfSlotReference+DURATION_tt1);
 ...
 radio_setFrequency(self, (self->ieee154e_vars).freq);
            ...
            sctimer_scheduleActionIn(ACTION_SEND_PACKET,  (self->ieee154e_vars).startOfSlotReference+DURATION_tt2);
                // 3. schedule timer radio tx watchdog
            sctimer_scheduleActionIn(ACTION_SET_TIMEOUT, (self->ieee154e_vars).startOfSlotReference+DURATION_tt3);
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
            // arm rt1
         sctimer_scheduleActionIn(ACTION_RADIORX_ENABLE,(self->ieee154e_vars).startOfSlotReference+DURATION_rt1);
            radio_rxPacket_prepare();
            // 2. schedule timer for starting
         sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,  (self->ieee154e_vars).startOfSlotReference+DURATION_rt2);
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm tt4
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,(self->ieee154e_vars).startOfSlotReference+DURATION_tt4);
#else
    // arm tt4
 opentimers_scheduleAbsolute(self, 
        (self->ieee154e_vars).timerId,                            // timerId
...
port_INLINE void activity_ti5(OpenMote* self, PORT_TIMER_WIDTH capturedTime) {
 ...
 radio_rfOff(self);
    ...
    if (listenForAck==TRUE) {
        ...
        // arm tt5
        sctimer_scheduleActionIn(ACTION_RADIORX_ENABLE,(self->ieee154e_vars).startOfSlotReference+DURATION_tt5);
        // set receiving buffer address (radio is NOT enabled at this moment)
        radio_rxPacket_prepare();
        // 2. schedule timer for starting receiving
        sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,(self->ieee154e_vars).startOfSlotReference+DURATION_tt6);
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm tt7
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,(self->ieee154e_vars).startOfSlotReference+DURATION_tt7);
#else
    // arm tt7
 opentimers_scheduleAbsolute(self, 
        (self->ieee154e_vars).timerId,                            // timerId
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm tt8
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,(self->ieee154e_vars).startOfSlotReference+DURATION_tt8);
#else
    // arm tt8
 opentimers_scheduleAbsolute(self, 
        (self->ieee154e_vars).timerId,                            // timerId
...
port_INLINE void activity_ti9(OpenMote* self, PORT_TIMER_WIDTH capturedTime) {
 ...
 radio_getReceivedFrame(self, 
            (self->ieee154e_vars).ackReceived->payload,
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm rt3
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,(self->ieee154e_vars).startOfSlotReference+DURATION_rt3);
#else
    // arm rt3
 opentimers_scheduleAbsolute(self, 
        (self->ieee154e_vars).timerId,                            // timerId
...
port_INLINE void activity_ri4(OpenMote* self, PORT_TIMER_WIDTH capturedTime) {
 ...
 opentimers_scheduleAbsolute(self, 
    ...
    (self->ieee154e_vars).syncCapturedTime = capturedTime;
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,(self->ieee154e_vars).startOfSlotReference+DURATION_rt4);
#else
 opentimers_scheduleAbsolute(self, 
        (self->ieee154e_vars).timerId,                            // timerId
...
port_INLINE void activity_ri5(OpenMote* self, PORT_TIMER_WIDTH capturedTime) {
 ...
 packetfunctions_reserveFooterSize(self, (self->ieee154e_vars).ackToSend,2);
            // 1. schedule timer for loading packet
            sctimer_scheduleActionIn(ACTION_LOAD_PACKET,(self->ieee154e_vars).startOfSlotReference+DURATION_rt5);
            ...
            sctimer_scheduleActionIn(ACTION_SEND_PACKET,(self->ieee154e_vars).startOfSlotReference+DURATION_rt6);
            // 2. schedule timer for radio tx watchdog
            sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,(self->ieee154e_vars).startOfSlotReference+DURATION_rt7);
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm rt8
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,(self->ieee154e_vars).startOfSlotReference+DURATION_rt8);
#else
    // arm rt8
 opentimers_scheduleAbsolute(self, 
        (self->ieee154e_vars).timerId,                            // timerId
...
port_INLINE void incrementAsnOffset(OpenMote* self) {
   ...
   (self->ieee154e_vars).asnOffset   = ((self->ieee154e_vars).asnOffset+1)% NUM_CHANNELS;
}
...
void endSlot(OpenMote* self) {
 ...
 opentimers_scheduleAbsolute(self, 
    ...
    if (DURATION_si+(self->ieee154e_vars).startOfSlotReference- opentimers_getValue(self)<(self->ieee154e_vars).slotDuration) {
 openserial_inhibitStop(self); // end of slot
    }
}
