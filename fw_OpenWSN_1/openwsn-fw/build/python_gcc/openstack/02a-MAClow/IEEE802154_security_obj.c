...
This was done on 2022-01-19 11:45:50.588000.
*/
/**
...
// declaration of global variable _ieee802154_security_vars_ removed during objectification.
...
void IEEE802154_security_init(OpenMote* self) {
   ...
   // this flag to true by calling IEEE802154_security_setDynamicKeying(self)
   (self->ieee802154_security_vars).dynamicKeying = FALSE;

    // TODO joinPermitted flag should be set dynamically upon a button press
    // and propagated through the network via EBs
    (self->ieee802154_security_vars).joinPermitted = TRUE;

   // invalidate beacon key (key 1)
   (self->ieee802154_security_vars).k1.index = IEEE802154_SECURITY_KEYINDEX_INVALID;
   memset(&(self->ieee802154_security_vars).k1.value[0], 0x00, 16);

   // invalidate data key (key 2)
   (self->ieee802154_security_vars).k2.index = IEEE802154_SECURITY_KEYINDEX_INVALID;
   memset(&(self->ieee802154_security_vars).k2.value[0], 0x00, 16);
      }

uint8_t IEEE802154_security_getBeaconKeyIndex(OpenMote* self) {
    return (self->ieee802154_security_vars).k1.index;
}
uint8_t IEEE802154_security_getDataKeyIndex(OpenMote* self) {
    return (self->ieee802154_security_vars).k2.index;
}

void IEEE802154_security_setBeaconKey(OpenMote* self, uint8_t index, uint8_t* value) {
    (self->ieee802154_security_vars).k1.index = index;
    memcpy((self->ieee802154_security_vars).k1.value, value, 16);
}

void IEEE802154_security_setDataKey(OpenMote* self, uint8_t index, uint8_t* value) {
    (self->ieee802154_security_vars).k2.index = index;
    memcpy((self->ieee802154_security_vars).k2.value, value, 16);
}

bool IEEE802154_security_isConfigured(OpenMote* self) {
    if ((self->ieee802154_security_vars).dynamicKeying == FALSE) {
        return TRUE;
    }

    if ((self->ieee802154_security_vars).k1.index != IEEE802154_SECURITY_KEYINDEX_INVALID &&
         (self->ieee802154_security_vars).k2.index != IEEE802154_SECURITY_KEYINDEX_INVALID) {
        return TRUE;
    }
    return FALSE;
...
void IEEE802154_security_setDynamicKeying(OpenMote* self) {
    (self->ieee802154_security_vars).dynamicKeying = TRUE;
}
...
void IEEE802154_security_prependAuxiliarySecurityHeader(OpenMote* self, OpenQueueEntry_t* msg){
 ...
 packetfunctions_reserveHeaderSize(self, msg, sizeof(uint8_t));
      ...
      case IEEE154_ASH_KEYIDMODE_EXPLICIT_64: //keySource with 64b address
         temp_keySource = &msg->l2_keySource;
 packetfunctions_writeAddress(self, msg,temp_keySource,OW_LITTLE_ENDIAN);
         break;
      default://error
 openserial_printError(self, COMPONENT_SECURITY,ERR_SECURITY,
...
owerror_t IEEE802154_security_outgoingFrameSecurity(OpenMote* self, OpenQueueEntry_t*   msg){
   ...
   key = msg->l2_frameType == IEEE154_TYPE_BEACON ? (self->ieee802154_security_vars).k1.value : (self->ieee802154_security_vars).k2.value;

   // First 8 bytes of the nonce are always the source address of the frame
   memcpy(&nonce[0], idmanager_getMyID(self, ADDR_64B)->addr_64b,8);

   // Fill last 5 bytes with the ASN part of the nonce
 ieee154e_getAsn(self, &nonce[8]);
 ...
 packetfunctions_reserveFooterSize(self, msg,msg->l2_authenticationLength);
   }

   //Encryption and/or authentication
   // cryptoengine overwrites m[] with ciphertext and appends the MIC
   outStatus = cryptoengine_aes_ccms_enc(self, a,
                                          len_a,
                                          m,
                                          &len_m,
                                          nonce,
...
*/
void IEEE802154_security_retrieveAuxiliarySecurityHeader(OpenMote* self, OpenQueueEntry_t*      msg, 
 ...
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload)+tempheader->headerLength),
                                     ADDR_16B,
                                     &msg->l2_keySource,
                                     OW_LITTLE_ENDIAN);
...
owerror_t IEEE802154_security_incomingFrame(OpenMote* self, OpenQueueEntry_t* msg){
 ...
 openserial_printError(self, COMPONENT_SECURITY,ERR_SECURITY,
   ...
   outStatus = cryptoengine_aes_ccms_dec(self, a,
                                          len_a,
                                          c,
                                          &len_c,
                                          nonce,
 ...
 packetfunctions_tossFooter(self, msg,msg->l2_authenticationLength);
...
uint8_t IEEE802154_security_getSecurityLevel(OpenMote* self, OpenQueueEntry_t *msg) {
    ...
    if( neighbors_isInsecureNeighbor(self, &msg->l2_nextORpreviousHop) &&
       (self->ieee802154_security_vars).joinPermitted == TRUE) {
        return IEEE154_ASH_SLF_TYPE_NOSEC;
      }
...
bool IEEE802154_security_acceptableLevel(OpenMote* self, OpenQueueEntry_t* msg, ieee802154_header_iht* parsedHeader) {
    ...
    if ( IEEE802154_security_isConfigured(self)               == TRUE &&
        msg->l2_securityLevel == IEEE154_ASH_SLF_TYPE_NOSEC      &&
        (self->ieee802154_security_vars).joinPermitted           == TRUE &&
 neighbors_isInsecureNeighbor(self, &parsedHeader->src) == TRUE) {
        return TRUE;
             }
