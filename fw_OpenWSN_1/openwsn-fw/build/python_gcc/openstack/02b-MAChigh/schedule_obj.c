...
This was done on 2022-01-19 11:46:00.815000.
*/
...
void schedule_removeAllManagedUnicastCellsToNeighbor(OpenMote* self, 
    uint8_t        slotframeID,
    open_addr_t*   neighbor
    ){
    uint8_t i;
    ...
    for(i=0;i<MAXACTIVESLOTS;i++){
        if (
 packetfunctions_sameAddress(self, &((self->schedule_vars).scheduleBuf[i].neighbor),neighbor) &&
            (
                (self->schedule_vars).scheduleBuf[i].type == CELLTYPE_TX ||
                (self->schedule_vars).scheduleBuf[i].type == CELLTYPE_RX
            )
        ){
...
void schedule_removeAllAutonomousTxRxCellUnicast(OpenMote* self){
    uint8_t i;

    // remove all entries in schedule with previousHop address
    for(i=0;i<MAXACTIVESLOTS;i++){
        if (
            (self->schedule_vars).scheduleBuf[i].type          == CELLTYPE_TXRX &&
            (self->schedule_vars).scheduleBuf[i].shared                         &&
            (self->schedule_vars).scheduleBuf[i].neighbor.type == ADDR_64B
        ){
 schedule_removeActiveSlot(self, 
              (self->schedule_vars).scheduleBuf[i].slotOffset,
              &((self->schedule_vars).scheduleBuf[i].neighbor)
           );
        }
    }
 msf_setHashCollisionFlag(self, FALSE);
}
...
uint8_t schedule_getNumberOfManagedTxCells(OpenMote* self, open_addr_t* neighbor){
    uint8_t i;
    uint8_t counter;
    ...
    for(i=0;i<MAXACTIVESLOTS;i++) {
        if(
            (self->schedule_vars).scheduleBuf[i].shared == FALSE       &&
            (self->schedule_vars).scheduleBuf[i].type   == CELLTYPE_TX &&
...
bool schedule_hasAutonomousTxRxCellUnicast(OpenMote* self, open_addr_t* neighbor){
    uint8_t i;
...
bool schedule_getAutonomousTxRxCellUnicastNeighbor(OpenMote* self, open_addr_t* neighbor){
    uint8_t i;
 ...
 packetfunctions_sameAddress(self, neighbor,&(self->schedule_vars).scheduleBuf[i].neighbor)
            ...
            memcpy(neighbor, &(self->schedule_vars).scheduleBuf[i].neighbor, sizeof(open_addr_t));
            ENABLE_INTERRUPTS();
            return TRUE;
        }
    }
...
bool schedule_hasManagedTxCellToNeighbor(OpenMote* self, open_addr_t* neighbor){
    uint8_t i;
...
bool schedule_hasNonParentAutonomousTxRxCellUnicast(OpenMote* self, open_addr_t* parentNeighbor, open_addr_t* nonParentNeighbor){
    uint8_t i;
    ...
    for(i=0;i<MAXACTIVESLOTS;i++) {
        if(
            ...
            (self->schedule_vars).scheduleBuf[i].shared                         &&
            (self->schedule_vars).scheduleBuf[i].neighbor.type == ADDR_64B      &&
 packetfunctions_sameAddress(self, parentNeighbor,&(self->schedule_vars).scheduleBuf[i].neighbor) == FALSE
        ){
            memcpy(nonParentNeighbor,&(self->schedule_vars).scheduleBuf[i].neighbor,sizeof(open_addr_t));
            ENABLE_INTERRUPTS();
            return TRUE;
        }
    }
...
bool schedule_getAutonomousTxRxCellAnycast(OpenMote* self, uint16_t* slotoffset){
    uint8_t i;
    ...
    for(i=0;i<MAXACTIVESLOTS;i++) {
        if(
            (self->schedule_vars).scheduleBuf[i].type          == CELLTYPE_TXRX &&
            (self->schedule_vars).scheduleBuf[i].shared        == FALSE         &&
            (self->schedule_vars).scheduleBuf[i].neighbor.type == ADDR_ANYCAST
        ){
            *slotoffset = (self->schedule_vars).scheduleBuf[i].slotOffset;
            ENABLE_INTERRUPTS();
            return TRUE;
        }
    }
...
*/
bool schedule_getShared(OpenMote* self) {
    bool returnVal;

    INTERRUPT_DECLARATION();
    DISABLE_INTERRUPTS();

    returnVal = (self->schedule_vars).currentScheduleEntry->shared;
