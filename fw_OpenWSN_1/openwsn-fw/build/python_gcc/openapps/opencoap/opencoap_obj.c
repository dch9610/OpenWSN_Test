...
This was done on 2022-01-19 11:46:41.092000.
*/
...
//=========================== prototype =======================================
void opencoap_header_encode(OpenMote* self, OpenQueueEntry_t *msg,
        uint8_t version,
        coap_type_t type,
        uint8_t TKL,
        coap_code_t code,
...
void opencoap_handle_proxy_scheme(OpenMote* self, OpenQueueEntry_t *msg,
        coap_header_iht* header,
        coap_option_iht* incomingOptions,
        uint8_t incomingOptionsLen);

void opencoap_handle_stateless_proxy(OpenMote* self, OpenQueueEntry_t *msg,
        coap_header_iht* header,
        coap_option_iht* incomingOptions,
        uint8_t incomingOptionsLen);

void opencoap_add_stateless_proxy_option(OpenMote* self, coap_option_iht* option,
        uint8_t* address,
        uint8_t addressLen,
        uint16_t portNumber);

void opencoap_forward_message(OpenMote* self, OpenQueueEntry_t *msg,
        coap_header_iht* header,
...
void opencoap_init(OpenMote* self) {
   ...
   while (pos<16) {
       rand = openrandom_get16b(self);
       memcpy(&(self->opencoap_vars).statelessProxy.key[pos],&rand,2);
       pos+=2;
   }
   // init sequence number to zero
   (self->opencoap_vars).statelessProxy.sequenceNumber = 0;

   // register at UDP stack
   (self->opencoap_vars).desc.port              = WKP_UDP_COAP;
   (self->opencoap_vars).desc.callbackReceive   = &opencoap_receive;
   (self->opencoap_vars).desc.callbackSendDone  = opencoap_sendDone;
 openudp_register(self, &(self->opencoap_vars).desc);
}
...
void opencoap_receive(OpenMote* self, OpenQueueEntry_t* msg) {
 ...
 packetfunctions_tossHeader(self, msg,index);
   ...
   index = opencoap_options_parse(self, &msg->payload[0], msg->length, coap_incomingOptions, &coap_incomingOptionsLen);
 ...
 packetfunctions_tossHeader(self, msg,index);
   ...
   if (statelessProxy) {
 opencoap_handle_stateless_proxy(self, msg, &coap_header, coap_incomingOptions, coap_incomingOptionsLen);
 openqueue_freePacketBuffer(self, msg);
       return;
   }
   ...
   if (proxyScheme) {
 opencoap_handle_proxy_scheme(self, msg, &coap_header, coap_incomingOptions, coap_incomingOptionsLen);
 openqueue_freePacketBuffer(self, msg);
        return;
   }
   ...
   if (objectSecurity) {
       ...
       if (objectSecurity->length == 0) {
           index = openoscoap_parse_compressed_COSE(self, &msg->payload[0],
                   msg->length,
                   &rcvdSequenceNumber,
                   &rcvdKid,
                   &rcvdKidLen);
 ...
 packetfunctions_tossHeader(self, msg, index);
           ...
           index = openoscoap_parse_compressed_COSE(self, objectSecurity->pValue,
   ...
   if (
      ...
      if (objectSecurity) {
            ...
            if (blindContext) {
                    coap_incomingOptionsLen = MAX_COAP_OPTIONS;
                    decStatus = openoscoap_unprotect_message(self, blindContext,
                            coap_header.Ver,
                            coap_header.Code,
         ...
         if (
            ...
            if (found==TRUE && temp_desc->callbackRx!=NULL) {
                if (temp_desc->securityContext != NULL && coap_header.Code < COAP_CODE_RESP_BADREQ) {
                    coap_incomingOptionsLen = MAX_COAP_OPTIONS;
                    decStatus = openoscoap_unprotect_message(self, temp_desc->securityContext,
                            coap_header.Ver,
                            coap_header.Code,
                            coap_incomingOptions,
                            &coap_incomingOptionsLen,
               ...
               temp_desc->callbackRx(self, msg,&coap_header,&coap_incomingOptions[0], NULL, NULL);
            }
         }
 ...
 openqueue_freePacketBuffer(self, msg);
   ...
   if (found==TRUE && securityReturnCode==COAP_CODE_EMPTY) {
      ...
      outcome = temp_desc->callbackRx(self, msg,&coap_header,&coap_incomingOptions[0], coap_outgoingOptions, &coap_outgoingOptionsLen);
      if (outcome == E_FAIL) {
 ...
 openoscoap_protect_message(self, 
 ...
 opencoap_options_encode(self, msg, coap_outgoingOptions, coap_outgoingOptionsLen, class);
   ...
   // fill in CoAP header
 opencoap_header_encode(self, msg,
           COAP_VERSION,
           response_type,
           coap_header.TKL,
           coap_header.Code,
...
*/
void opencoap_writeLinks(OpenMote* self, OpenQueueEntry_t* msg, uint8_t componentID) {
   coap_resource_desc_t* temp_resource;
...
owerror_t opencoap_send(OpenMote* self, 
   ...
   if ((self->opencoap_vars).messageID++ == 0xffff) {
      (self->opencoap_vars).messageID = 0;
   }
   ...
   request->T                       = type;
   request->Code                    = code;
   request->messageID               = (self->opencoap_vars).messageID;
   request->TKL                     = TKL<COAP_MAX_TKL ? TKL : COAP_MAX_TKL;

   while (tokenPos<request->TKL) {
       token = openrandom_get16b(self);
   ...
   if (descSender->securityContext != NULL) { // security activated for the resource
      ...
      request->oscoapSeqNum = openoscoap_get_sequence_number(self, descSender->securityContext);
      // protect the message in the openqueue buffer
      ret = openoscoap_protect_message(self, 
 ...
 packetfunctions_reserveHeaderSize(self, msg, 1);
   ...
   // once header is reserved, encode the options to the openqueue payload buffer
 opencoap_options_encode(self, msg,
           options,
           optionsLen,
           class);

   // pre-pend CoAP header (version,type,TKL,code,messageID,Token)
 opencoap_header_encode(self, msg, COAP_VERSION, type, request->TKL, code, request->messageID, request->token);
...
owerror_t opencoap_options_encode(OpenMote* self, 
        OpenQueueEntry_t*       msg,
        coap_option_iht*        options,
        uint8_t                 optionsLen,
        coap_option_class_t     class
    ...
    if (options != NULL && optionsLen != 0) {
        for (i = optionsLen ; i-- > 0 ; ) {
            ...
            if (delta <= 12) {
                ...
                optionDelta = 14;
 packetfunctions_htons(self, (uint16_t) delta - 269, optionDeltaExt);
                optionDeltaExtLen = 2;
            }
            else {
                return E_FAIL;
 ...
 packetfunctions_reserveHeaderSize(self, msg, options[i].length);
            memcpy(&msg->payload[0], options[i].pValue, options[i].length);

 packetfunctions_reserveHeaderSize(self, msg, optionLengthExtLen);
            memcpy(&msg->payload[0], optionLengthExt, optionLengthExtLen);

 packetfunctions_reserveHeaderSize(self, msg, optionDeltaExtLen);
...
uint8_t opencoap_options_parse(OpenMote* self, 
        uint8_t*                buffer,
        uint8_t                 bufferLen,
        coap_option_iht*        options,
        uint8_t*                optionsLen
    ...
    for (i = 0; i < *optionsLen; i++) {
        ...
        if (optionDelta <= 12) {
            ...
            optionDelta = (coap_option_t) ( packetfunctions_ntohs(self, &buffer[index]) + 269);
            index += 2;
        }
        else {
            break;
...
void opencoap_handle_proxy_scheme(OpenMote* self, OpenQueueEntry_t *msg,
 ...
 opencoap_add_stateless_proxy_option(self, &outgoingOptions[outgoingOptionsLen++],
    ...
    if ( icmpv6rpl_getRPLDODAGid(self, JRCaddress.addr_128b) == E_SUCCESS) {
 opencoap_forward_message(self, msg, header, outgoingOptions, outgoingOptionsLen, &JRCaddress, WKP_UDP_COAP);
    }
    return;
}
...
void opencoap_handle_stateless_proxy(OpenMote* self, OpenQueueEntry_t *msg,
 ...
 packetfunctions_mac64bToIp128b(self, &link_local_prefix, &eui64, &destIP);

    if (statelessProxy->length == 10) {
        portNumber = packetfunctions_ntohs(self, &statelessProxy->pValue[8]);
    }
 ...
 opencoap_forward_message(self, msg, header, outgoingOptions, outgoingOptionsLen, &destIP, portNumber);
}
...
void opencoap_header_encode(OpenMote* self, OpenQueueEntry_t *msg,
        ...
        uint8_t *token) {
   // pre-pend CoAP header (version,type,TKL,code,messageID,Token)
 packetfunctions_reserveHeaderSize(self, msg,4+TKL);
...
void opencoap_add_stateless_proxy_option(OpenMote* self, coap_option_iht* option,
    ...
    memcpy(&(self->opencoap_vars).statelessProxy.buffer[len], address, addressLen);
    len += addressLen;

    if (portNumber != WKP_UDP_COAP) {
 packetfunctions_htons(self, portNumber, &(self->opencoap_vars).statelessProxy.buffer[len]);
        len += 2;
    }

    option->type = COAP_OPTION_NUM_STATELESSPROXY;
    option->length = len;
    option->pValue = (self->opencoap_vars).statelessProxy.buffer;
}
...
void opencoap_forward_message(OpenMote* self, OpenQueueEntry_t *msg,
    ...
    outgoingPacket = openqueue_getFreePacketBuffer(self, COMPONENT_OPENCOAP);
 ...
 openserial_printError(self, 
    ...
    // fill payload
    if (msg->length) {
 packetfunctions_reserveHeaderSize(self, outgoingPacket,msg->length);
        memcpy(outgoingPacket->payload, msg->payload, msg->length);
 packetfunctions_reserveHeaderSize(self, outgoingPacket, 1);
        outgoingPacket->payload[0] = COAP_PAYLOAD_MARKER;
    }

    // encode options
 opencoap_options_encode(self, outgoingPacket, outgoingOptions, outgoingOptionsLen, COAP_OPTION_CLASS_ALL);

    // encode CoAP header
 opencoap_header_encode(self, outgoingPacket,
            header->Ver,
            header->T,
            header->TKL,
            header->Code,
    ...
    if (( openudp_send(self, outgoingPacket))==E_FAIL) {
 openqueue_freePacketBuffer(self, outgoingPacket);
    }
}
