...
This was done on 2022-01-19 11:46:48.203000.
*/
/**
...
// declaration of global variable _cinfo_vars_ removed during objectification.
...
void cinfo_init(OpenMote* self) {
   ...
   (self->cinfo_vars).desc.path0len             = sizeof(cinfo_path0)-1;
   (self->cinfo_vars).desc.path0val             = (uint8_t*)(&cinfo_path0);
   (self->cinfo_vars).desc.path1len             = 0;
   (self->cinfo_vars).desc.path1val             = NULL;
   (self->cinfo_vars).desc.componentID          = COMPONENT_CINFO;
   (self->cinfo_vars).desc.securityContext      = NULL;
   (self->cinfo_vars).desc.discoverable         = TRUE;
   (self->cinfo_vars).desc.callbackRx           = &cinfo_receive;
   (self->cinfo_vars).desc.callbackSendDone     = &cinfo_sendDone;
   
   // register with the CoAP module
 opencoap_register(self, &(self->cinfo_vars).desc);
}
...
owerror_t cinfo_receive(OpenMote* self, 
 ...
 packetfunctions_reserveHeaderSize(self, msg,1);
         msg->payload[0] = '\n';
 packetfunctions_reserveHeaderSize(self, msg,sizeof(infoStackName)-1+6);
