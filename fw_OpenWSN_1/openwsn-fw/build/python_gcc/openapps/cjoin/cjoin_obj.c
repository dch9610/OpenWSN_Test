...
This was done on 2022-01-19 11:46:49.036000.
*/
/**
...
//    (self->cjoin_vars).timerId = opentimers_create(self, TIMER_GENERAL_PURPOSE, TASKPRIO_COAP);
...
void cjoin_init_security_context(OpenMote* self) {
 ...
 eui64_get(self, senderID);
   senderID[8] = 0x00;      // EUI-64 || 0x00 [minimal-security-03]

 eui64_get(self, recipientID);
...
owerror_t cjoin_receive(OpenMote* self, OpenQueueEntry_t* msg,
    ...
    if (configuration.keyset.num_keys == 1 &&
        configuration.keyset.key[0].key_usage == COJP_KEY_USAGE_6TiSCH_K1K2_ENC_MIC32) {
            ...
            if ((self->cjoin_vars).isJoined==FALSE){
 neighbor_removeAutonomousTxRxCellUnicast(self, &(msg->l2_nextORpreviousHop));
            }
...
void cjoin_retransmission_cb(OpenMote* self, opentimers_id_t id) {
    // calling the task directly as the timer_cb function is executed in
    // task mode by opentimer already
 cjoin_retransmission_task_cb(self);
}
