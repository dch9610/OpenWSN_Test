...
This was done on 2022-01-19 11:47:21.724000.
*/
/**
...
static owerror_t aes_cbc_mac(OpenMote* self, uint8_t* a, uint8_t len_a, uint8_t* m, uint8_t len_m, uint8_t* nonce, uint8_t key[16], uint8_t* mac, uint8_t len_mac, uint8_t l);
static owerror_t aes_ctr_enc(OpenMote* self, uint8_t* m, uint8_t len_m, uint8_t* nonce, uint8_t key[16], uint8_t* mac, uint8_t len_mac, uint8_t l);
owerror_t aes_cbc_enc_raw(OpenMote* self, uint8_t* buffer, uint8_t len, uint8_t key[16], uint8_t iv[16]); 
owerror_t aes_ctr_enc_raw(OpenMote* self, uint8_t* buffer, uint8_t len, uint8_t key[16], uint8_t iv[16]);
...
*/
owerror_t openccms_enc(OpenMote* self, uint8_t* a,
         uint8_t len_a,
         uint8_t* m,
         uint8_t* len_m,
         uint8_t* nonce,
   ...
   if ( aes_cbc_mac(self, a, len_a, m, *len_m, nonce, key, mac, len_mac, l) == E_SUCCESS) {
      if ( aes_ctr_enc(self, m, *len_m, nonce, key, mac, len_mac, l) == E_SUCCESS) {
...
*/
owerror_t openccms_dec(OpenMote* self, uint8_t* a,
         uint8_t len_a,
         uint8_t* m,
         uint8_t* len_m,
         uint8_t* nonce,
   ...
   *len_m -= len_mac;
   ...
   if ( aes_ctr_enc(self, m, *len_m, nonce, key, mac, len_mac, l) == E_SUCCESS) {
      if ( aes_cbc_mac(self, a, len_a, m, *len_m, nonce, key, orig_mac, len_mac, l) == E_SUCCESS) {
...
\returns E_SUCCESS when the generation was successful, E_FAIL otherwise. 
*/
static owerror_t aes_cbc_mac(OpenMote* self, uint8_t* a,
         uint8_t len_a,
         uint8_t* m,
         uint8_t len_m,
         uint8_t* nonce,
 ...
 aes_cbc_enc_raw(self, buffer, len, key, cbc_mac_iv);
...
*/
static owerror_t aes_ctr_enc(OpenMote* self, uint8_t* m,
         uint8_t len_m,
         uint8_t* nonce,
         uint8_t key[16],
         uint8_t* mac,
 ...
 aes_ctr_enc_raw(self, buffer, len, key, iv);
