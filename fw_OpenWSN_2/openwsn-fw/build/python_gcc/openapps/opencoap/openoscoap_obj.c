...
This was done on 2022-01-19 11:46:58.071000.
*/
...
// declaration of global variable _openoscoap_vars_ removed during objectification.
...
uint8_t openoscoap_construct_aad(OpenMote* self, uint8_t* buffer,
        uint8_t version,
        uint8_t code,
...
void openoscoap_encode_compressed_COSE(OpenMote* self, OpenQueueEntry_t* msg,
...
uint8_t openoscoap_convert_sequence_number(OpenMote* self, uint16_t sequenceNumber, uint8_t** buffer);
//=========================== public ==========================================
...
owerror_t openoscoap_protect_message(OpenMote* self, 
    ...
    requestSeqLen = openoscoap_convert_sequence_number(self, sequenceNumber, &requestSeq);
 ...
 packetfunctions_reserveHeaderSize(self, msg, AES_CCM_16_64_128_TAG_LEN);
    memcpy(&msg->payload[0], payload, payloadLen);
    // update payload pointer but leave length intact
    payload = &msg->payload[0];

    aadLen = openoscoap_construct_aad(self, aad,
            version,
            code,
            NULL,
 ...
 openserial_printError(self, 
    ...
    if (is_request(code)) {
     ...
     encStatus = cryptoengine_aes_ccms_enc(self, aad,
                                        aadLen,
                                        payload,
                                        &payloadLen,
                                        nonce,
 ...
 openoscoap_encode_compressed_COSE(self, msg, requestSeq, requestSeqLen, requestKid, requestKidLen);
 ...
 packetfunctions_tossHeader(self, msg, msg->length); // reset packet to zero as objectSecurity option will cary payload
    }
...
owerror_t openoscoap_unprotect_message(OpenMote* self, 
 ...
 openserial_printError(self, 
    ...
    decStatus = cryptoengine_aes_ccms_dec(self, aad,
                                    aadLen,
                                    ciphertext,
                                    &ciphertextLen,
                                    nonce,
 ...
 openserial_printError(self, 
    ...
    if (payloadInObjSec) {
 opencoap_options_parse(self, objectSecurity->pValue, objectSecurity->length, incomingOptions, incomingOptionsLen);
    }
    else {
 packetfunctions_tossFooter(self, msg, AES_CCM_16_64_128_TAG_LEN);
        index = opencoap_options_parse(self, &msg->payload[0], msg->length, incomingOptions, incomingOptionsLen);
...
uint8_t openoscoap_parse_compressed_COSE(OpenMote* self, uint8_t *buffer,
    ...
    if (pivsz > 2) {
        ...
        *sequenceNumber = packetfunctions_ntohs(self, &buffer[index]);
        index += 2;
    }
...
void openoscoap_encode_compressed_COSE(OpenMote* self, OpenQueueEntry_t* msg,
    ...
    if (kidFlag) {
 packetfunctions_reserveHeaderSize(self, msg, kidLen + 1);
        msg->payload[0] = kidLen;
        memcpy(&msg->payload[1], kid, kidLen);
    }

    if (partialIVLen) {
 packetfunctions_reserveHeaderSize(self, msg, partialIVLen);
...
uint8_t openoscoap_convert_sequence_number(OpenMote* self, uint16_t sequenceNumber, uint8_t** buffer) {
 packetfunctions_htons(self, sequenceNumber, *buffer);
    if (**buffer == 0x00) {
        (*buffer)++;
        return 1;
    }
