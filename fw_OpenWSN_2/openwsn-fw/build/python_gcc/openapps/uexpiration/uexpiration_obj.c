...
This was done on 2022-01-19 11:46:55.022000.
*/
#include "stdlib.h"
...
void uexpiration_timer_cb(OpenMote* self, opentimers_id_t id);
void uexpiration_task_cb(OpenMote* self);
...
void uexpiration_init(OpenMote* self) {
 ...
 openudp_register(self, &uexpiration_vars.desc);
}

void uexpiration_receive(OpenMote* self, OpenQueueEntry_t* request) {
   char buffer[5];
   char clr_buffer[5]= "     ";
   ...
   uexpiration_vars.timerId = opentimers_create(self, TIMER_GENERAL_PURPOSE, TASKPRIO_UDP);
...
void uexpiration_task_cb(OpenMote* self) {
   ...
   reply = openqueue_getFreePacketBuffer(self, COMPONENT_UEXPIRATION);
   if (reply==NULL) {
 ...
 packetfunctions_reserveHeaderSize(self, reply,sizeof(uint16_t));
 ...
 opentimers_destroy(self, uexpiration_vars.timerId);
   } else {
 opentimers_scheduleIn(self, 
           uexpiration_vars.timerId,
           uexpiration_vars.period,
