...
This was done on 2022-01-19 11:46:13.719000.
*/
...
// declaration of global variable _monitor_expiration_vars_ removed during objectification.
#endif

//=========================== prototypes ======================================

//===== IPv6 header
void iphc_retrieveIPv6Header(OpenMote* self, 
   OpenQueueEntry_t* msg, 
   ipv6_header_iht* ipv6_outer_header,
   ipv6_header_iht* ipv6_inner_header,
   uint8_t*         page_length
);
uint8_t iphc_retrieveIphcHeader(OpenMote* self, open_addr_t* temp_addr_16b,
...
void iphc_prependIPv6HopByHopHeader(OpenMote* self, 
   OpenQueueEntry_t*    msg,
   uint8_t              nextheader,
...
void iphc_prependIPv6DeadlineHeader(OpenMote* self, 
   OpenQueueEntry_t* msg
);

uint8_t iphc_getAsnLen(OpenMote* self, 
   uint8_t* asn
);
#endif
...
void iphc_init(OpenMote* self) {
}

// send from upper layer: I need to add 6LoWPAN header
owerror_t iphc_sendFromForwarding(OpenMote* self, 
    OpenQueueEntry_t* msg,
    ipv6_header_iht*  ipv6_outer_header,
    ipv6_header_iht*  ipv6_inner_header,
    rpl_option_ht*    rpl_option,
 ...
 packetfunctions_isAllRoutersMulticast(self, &(msg->l3_destinationAdd))==FALSE) {
 openserial_printCritical(self, COMPONENT_IPHC,ERR_BRIDGE_MISMATCH,
                            (errorparameter_t)0,
                            (errorparameter_t)0);
        return E_FAIL;
    }
 ...
 openserial_printError(self, COMPONENT_IPHC,ERR_HOP_LIMIT_REACHED,
                                (errorparameter_t)0,
                                (errorparameter_t)0);
            return E_FAIL;
        } else {
 ...
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),(open_addr_t*)dagroot_mac64b,&(temp_dagroot_ip128b));
            if (
                (
                  ipv6_outer_header->src.type == ADDR_NONE &&
 packetfunctions_sameAddress(self, &(msg->l3_sourceAdd),&(temp_dagroot_ip128b))
                ) || 
                (
                  ipv6_outer_header->src.type != ADDR_NONE &&
 packetfunctions_sameAddress(self, &(ipv6_outer_header->src),&(temp_dagroot_ip128b))
                 )
            ){
                // hop limit
 packetfunctions_reserveHeaderSize(self, msg,sizeof(uint8_t));
 ...
 packetfunctions_reserveHeaderSize(self, msg,sizeof(uint8_t));
                *((uint8_t*)(msg->payload)) = ELECTIVE_6LoRH | 1;
                ...
                if (sam == IPHC_SAM_128B){
                    // encapsulate address
 packetfunctions_writeAddress(self, msg, &(msg->l3_sourceAdd),OW_BIG_ENDIAN);
                    // hoplim
 packetfunctions_reserveHeaderSize(self, msg,sizeof(uint8_t));
                    *((uint8_t*)(msg->payload)) = ipv6_outer_header->hop_limit;
                    // type
 ...
 packetfunctions_isBroadcastMulticast(self, &(msg->l3_destinationAdd))==FALSE
        ){                    
 iphc_prependIPv6DeadlineHeader(self, msg);
        }
    }
#endif     
 ...
 iphc_prependIPv6HopByHopHeader(self, msg, msg->l4_protocol, rpl_option);
    }
    
    // copy RH3s back if length > 0
    if (rh3_length > 0){
 packetfunctions_reserveHeaderSize(self, msg,rh3_length);
 ...
 packetfunctions_reserveHeaderSize(self, msg,sizeof(uint8_t));
    ...
    return sixtop_send(self, msg);
}

//send from bridge: 6LoWPAN header already added by OpenLBR, send as is
owerror_t iphc_sendFromBridge(OpenMote* self, OpenQueueEntry_t *msg) {
   msg->owner = COMPONENT_IPHC;
   // error checking
...
void iphc_sendDone(OpenMote* self, OpenQueueEntry_t* msg, owerror_t error) {
   msg->owner = COMPONENT_IPHC;
   if (msg->creator==COMPONENT_OPENBRIDGE) {
 openbridge_sendDone(self, msg,error);
   } else {
 forwarding_sendDone(self, msg,error);
   }
}

void iphc_receive(OpenMote* self, OpenQueueEntry_t* msg) {
    ipv6_header_iht      ipv6_outer_header;
    ipv6_header_iht      ipv6_inner_header;
    uint8_t              page_length;
    rpl_option_ht        rpl_option;
    ...
    // then regular header
 iphc_retrieveIPv6Header(self, msg,&ipv6_outer_header,&ipv6_inner_header,&page_length);
    
    // if the address is broadcast address, the ipv6 header is the inner header
    if (
 idmanager_getIsDAGroot(self)==FALSE ||
 packetfunctions_isBroadcastMulticast(self, &(ipv6_inner_header.dest))
    ) {
 packetfunctions_tossHeader(self, msg,page_length);
        if (
 ...
 iphc_retrieveIPv6DeadlineHeader(self, 
                                  msg,
 ...
 openbridge_receive(self, msg);                   //out to the OpenVisualizer
   }
}
...
*/
owerror_t iphc_prependIPv6Header(OpenMote* self, 
      OpenQueueEntry_t* msg,
      uint8_t           tf,
 ...
 packetfunctions_writeAddress(self, msg,value_dest,OW_BIG_ENDIAN);
         }
         break;
      case IPHC_DAM_64B:
         if (m == IPHC_M_YES){
 ...
 packetfunctions_writeAddress(self, msg, ( idmanager_getMyID(self, ADDR_16B)),OW_BIG_ENDIAN);
         }
 ...
 packetfunctions_writeAddress(self, msg,value_src,OW_BIG_ENDIAN);
         }
         break;
      case IPHC_SAM_64B:
         if(fw_SendOrfw_Rcv==PCKTSEND) {
 packetfunctions_writeAddress(self, msg, ( idmanager_getMyID(self, ADDR_64B)),OW_BIG_ENDIAN);
         }
         if(fw_SendOrfw_Rcv==PCKTFORWARD) {
 ...
 packetfunctions_writeAddress(self, msg, value_src,OW_BIG_ENDIAN);
      ...
      case IPHC_SAM_128B:
         if(fw_SendOrfw_Rcv==PCKTSEND) {
 packetfunctions_writeAddress(self, msg, ( idmanager_getMyID(self, ADDR_64B)),OW_BIG_ENDIAN);
 packetfunctions_writeAddress(self, msg, ( idmanager_getMyID(self, ADDR_PREFIX)),OW_BIG_ENDIAN);
         }
         if(fw_SendOrfw_Rcv==PCKTFORWARD) {
...
*/
void iphc_retrieveIPv6Header(OpenMote* self, OpenQueueEntry_t* msg, ipv6_header_iht* ipv6_outer_header, ipv6_header_iht* ipv6_inner_header,uint8_t* page_length) {
    uint8_t         temp_8b;
 ...
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),(open_addr_t*)dagroot_mac64b,&(ipv6_outer_header->src));
    					// destination address is the first address in RH3 6LoRH OR dest adress in IPHC
    				} else{
    					switch(lorh_length-1){
    					case 16:
    						// this is source address
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_outer_header->header_length+*page_length+extention_header_length)),ADDR_128B,&ipv6_outer_header->src,OW_BIG_ENDIAN);
    						ipv6_outer_header->header_length += 16*sizeof(uint8_t);
    						break;
    					case 8:
    						// this is source address
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_outer_header->header_length+*page_length+extention_header_length)),ADDR_64B,&temp_addr_64b,OW_BIG_ENDIAN);
    						ipv6_outer_header->header_length += 8*sizeof(uint8_t);
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),&temp_addr_64b,&ipv6_outer_header->src);
    						break;
    					default:
...
uint8_t iphc_retrieveIphcHeader(OpenMote* self, open_addr_t* temp_addr_16b,
 ...
 openserial_printError(self, 
       ...
       switch (*sam) {
          case IPHC_SAM_ELIDED:
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),&(msg->l2_nextORpreviousHop),&ipv6_header->src);
             break;
          case IPHC_SAM_16B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length+previousLen)),ADDR_16B,temp_addr_16b,OW_BIG_ENDIAN);
             ipv6_header->header_length += 2*sizeof(uint8_t);
 packetfunctions_mac16bToMac64b(self, temp_addr_16b,temp_addr_64b);
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),temp_addr_64b,&ipv6_header->src);
             break;
          case IPHC_SAM_64B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length+previousLen)),ADDR_64B,temp_addr_64b,OW_BIG_ENDIAN);
             ipv6_header->header_length += 8*sizeof(uint8_t);
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),temp_addr_64b,&ipv6_header->src);
             break;
          case IPHC_SAM_128B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length+previousLen)),ADDR_128B,&ipv6_header->src,OW_BIG_ENDIAN);
 ...
 openserial_printError(self, 
           ...
           switch (*dam) {
              case IPHC_DAM_ELIDED:
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX), idmanager_getMyID(self, ADDR_64B),&(ipv6_header->dest));
                 break;
              case IPHC_DAM_16B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length+previousLen)),ADDR_16B,temp_addr_16b,OW_BIG_ENDIAN);
                 ipv6_header->header_length += 2*sizeof(uint8_t);
 packetfunctions_mac16bToMac64b(self, temp_addr_16b,temp_addr_64b);
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),temp_addr_64b,&ipv6_header->dest);
                 break;
              case IPHC_DAM_64B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length+previousLen)),ADDR_64B,temp_addr_64b,OW_BIG_ENDIAN);
                 ipv6_header->header_length += 8*sizeof(uint8_t);
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),temp_addr_64b,&ipv6_header->dest);
                 break;
              case IPHC_DAM_128B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length+previousLen)),ADDR_128B,&ipv6_header->dest,OW_BIG_ENDIAN);
                 ipv6_header->header_length += 16*sizeof(uint8_t);
                 break;
              default:
 openserial_printError(self, 
 ...
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),(open_addr_t*)dagroot_mac64b,&(ipv6_header->src));
                        // destination address is the first address in RH3 6LoRH OR dest adress in IPHC
                    } else{
...
void iphc_prependIPv6HopByHopHeader(OpenMote* self, 
 ...
 packetfunctions_reserveHeaderSize(self, msg,sizeof(uint16_t));
...
void iphc_prependIPv6DeadlineHeader(OpenMote* self, 
 ...
 ieee154e_getAsn(self, asn_array);
      
      asn_len = iphc_getAsnLen(self, asn_array);
      
 packetfunctions_reserveHeaderSize(self, msg,asn_len);
   ...
   // Expiration Time (ET)
 ieee154e_calculateExpTime(self, msg->max_delay,asn_array);    
   asn_len = iphc_getAsnLen(self, asn_array);
 packetfunctions_reserveHeaderSize(self, msg,asn_len);
   memcpy(&msg->payload[0],&asn_array,asn_len*sizeof(uint8_t));
   temp_len += asn_len;
...
*/
uint8_t iphc_retrieveIPv6HopByHopHeader(OpenMote* self, 
      OpenQueueEntry_t*      msg,
      rpl_option_ht*         rpl_option
   ){
   uint8_t temp_8b;
...
*/
void iphc_retrieveIPv6DeadlineHeader(OpenMote* self, 
      OpenQueueEntry_t*      msg,
 ...
 ieee154e_orderToASNStructure(self, deadline_option->et_val, &exp_asn); 
			 
       // Calculate delay experienced by packet
 ieee154e_getAsn(self, my_asn_array);
 ieee154e_orderToASNStructure(self, my_asn_array, &my_asn);			 
 ...
 openserial_printError(self, 
       ...
       (self->monitor_expiration_vars).time_left = deadline_option->time_left;      
 ...
 ieee154e_orderToASNStructure(self, deadline_option->ot_val, &my_asn); 
           (self->monitor_expiration_vars).time_elapsed = ieee154e_asnDiff(self,  &my_asn);	
       }                   
...
void iphc_getDeadlineInfo(OpenMote* self, monitor_expiration_vars_t*	stats) {
   stats->time_left = (self->monitor_expiration_vars).time_left;
   stats->time_elapsed = (self->monitor_expiration_vars).time_elapsed;
   memset(&((self->monitor_expiration_vars)),0,sizeof(monitor_expiration_vars_t));
}

uint8_t iphc_getAsnLen(OpenMote* self, uint8_t* asn) {
   uint8_t i;
   for(i=5; i>=1; i--) {
       if(asn[i-1] != 0) {
          return (i);
