...
This was done on 2022-01-19 11:46:31.683000.
*/
...
// declaration of global variable _openudp_vars_ removed during objectification.

//=========================== prototypes ======================================

static void openudp_sendDone_default_handler(OpenMote* self, OpenQueueEntry_t* msg, owerror_t error);
static void openudp_receive_default_handler(OpenMote* self, OpenQueueEntry_t* msg);

//=========================== public ==========================================

void openudp_init(OpenMote* self) {
   // initialize the resource linked list
   (self->openudp_vars).resources = NULL;
}

void openudp_register(OpenMote* self, udp_resource_desc_t* desc) {
   // chain the new resource to head of resource list
   desc->next = (self->openudp_vars).resources;
   (self->openudp_vars).resources = desc;
}
...
owerror_t openudp_send(OpenMote* self, OpenQueueEntry_t* msg) {
 ...
 packetfunctions_reserveHeaderSize(self, msg,2);
 packetfunctions_htons(self, msg->l4_destination_port,&(msg->payload[0]));
 packetfunctions_reserveHeaderSize(self, msg,1);
 ...
 packetfunctions_calculateChecksum(self, msg,checksum_position);
...
void openudp_sendDone(OpenMote* self, OpenQueueEntry_t* msg, owerror_t error) {
   ...
   resource = (self->openudp_vars).resources;
   while (NULL != resource) {
 ...
 udp_send_done_callback_ptr(self, msg, error);
}
...
void openudp_receive(OpenMote* self, OpenQueueEntry_t* msg) {
 ...
 packetfunctions_tossHeader(self, msg,1+2);
            break;
         case NHC_UDP_PORTS_4S_4D:
 ...
 udp_receive_done_callback_ptr(self, msg);  
   }
}
