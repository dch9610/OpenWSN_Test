...
This was done on 2022-01-19 11:46:20.770000.
*/
...
void forwarding_getNextHop(OpenMote* self, 
...
void forwarding_createRplOption(OpenMote* self, 
   rpl_option_ht*       rpl_option,
   uint8_t              flags
);
...
owerror_t forwarding_send(OpenMote* self, OpenQueueEntry_t* msg) {
 ...
 packetfunctions_ip128bToMac64b(self, &(msg->l3_destinationAdd),&temp_dest_prefix,&temp_dest_mac64b);
    ...
    if ( packetfunctions_isLinkLocal(self, &msg->l3_destinationAdd)                ||
 packetfunctions_isAllRoutersMulticast(self, &msg->l3_destinationAdd)  ||
 packetfunctions_isAllHostsMulticast(self, &msg->l3_destinationAdd)) {
    ...
    } else if ( packetfunctions_sameAddress(self, &temp_dest_prefix, idmanager_getMyID(self, ADDR_PREFIX))==FALSE && packetfunctions_isBroadcastMulticast(self, &(msg->l3_destinationAdd))==FALSE) {
 ...
 forwarding_createRplOption(self, 
    ...
    if ( packetfunctions_sameAddress(self, &temp_dest_prefix, myprefix)) {
...
void forwarding_sendDone(OpenMote* self, OpenQueueEntry_t* msg, owerror_t error) {
 ...
 openqueue_freePacketBuffer(self, msg);
      ...
      switch(msg->l4_protocol) {
         case IANA_UDP:
 openudp_sendDone(self, msg,error);
            break;
         case IANA_ICMPv6:
 icmpv6_sendDone(self, msg,error);
            break;
         default:
...
void forwarding_receive(OpenMote* self, 
 ...
 packetfunctions_tossHeader(self, msg,ipv6_outer_header->header_length + ipv6_outer_header->rhe_length);
        }
 ...
 openserial_printError(self, 
            ...
            if (senderRank < icmpv6rpl_getMyDAGrank(self)){
                // loop detected
                // set flag
 ...
 openserial_printError(self, 
                    ...
                    ERR_LOOP_DETECTED,
                    (errorparameter_t) senderRank,
                    (errorparameter_t) icmpv6rpl_getMyDAGrank(self)
                );
            }
...
*/
void forwarding_getNextHop(OpenMote* self, open_addr_t* destination128b, open_addr_t* addressToWrite64b) {
   uint8_t         i;

   if ( packetfunctions_isBroadcastMulticast(self, destination128b)) {
 ...
 icmpv6rpl_getPreferredParentEui64(self, addressToWrite64b);
   }
}
...
owerror_t forwarding_send_internal_RoutingTable(OpenMote* self, 
    ...
    if (
        ...
        if ( neighbors_isStableNeighbor(self, &(msg->l3_destinationAdd)) || msg->is_cjoin_response) {
            // IP destination is 1-hop neighbor, send directly
 packetfunctions_ip128bToMac64b(self, &(msg->l3_destinationAdd),&temp_prefix64btoWrite,&(msg->l2_nextORpreviousHop));
        }
    } else {
...
owerror_t forwarding_send_internal_SourceRouting(OpenMote* self, 
 ...
 packetfunctions_tossHeader(self, msg, sizeRH+2);
    	RH_length += 2 + sizeRH;
        temp_8b = *((uint8_t*)(msg->payload)+hlen);
        type    = *((uint8_t*)(msg->payload)+hlen+1);
    }
...
void forwarding_createRplOption(OpenMote* self, rpl_option_ht* rpl_option, uint8_t flags) {
    ...
    rpl_option->senderRank         = icmpv6rpl_getMyDAGrank(self);
