...
This was done on 2022-01-19 11:46:05.018000.
*/
...
void sixtop_init(OpenMote* self) {
    ...
    (self->sixtop_vars).ebSendingTimerId   = opentimers_create(self, TIMER_GENERAL_PURPOSE, TASKPRIO_SIXTOP);
 opentimers_scheduleIn(self, 
    ...
    (self->sixtop_vars).maintenanceTimerId   = opentimers_create(self, TIMER_GENERAL_PURPOSE, TASKPRIO_SIXTOP);
 opentimers_scheduleIn(self, 
    ...
    (self->sixtop_vars).timeoutTimerId      = opentimers_create(self, TIMER_GENERAL_PURPOSE, TASKPRIO_SIXTOP);
}
...
owerror_t sixtop_request(OpenMote* self, 
    ...
    if ( openqueue_getNum6PReq(self, neighbor)>0){
        // remove previous request as it's not sent out
 openqueue_remove6PrequestToNeighbor(self, neighbor);
    }
...
owerror_t sixtop_send(OpenMote* self, OpenQueueEntry_t *msg) {
 ...
 icmpv6rpl_getPreferredParentEui64(self, &addressToWrite)           &&
                    (
 msf_getHashCollisionFlag(self) == FALSE                      &&
 schedule_hasAutonomousTxRxCellUnicast(self, &addressToWrite)== FALSE
                    )
                )
            )
        )
...
void sixtop_sendingEb_timer_cb(OpenMote* self, opentimers_id_t id){
 ...
 timer_sixtop_sendEb_fired(self);
}
...
*/
void sixtop_maintenance_timer_cb(OpenMote* self, opentimers_id_t id) {
 timer_sixtop_management_fired(self);
}
...
*/
void sixtop_timeout_timer_cb(OpenMote* self, opentimers_id_t id) {
 timer_sixtop_six2six_timeout_fired(self);
}
...
void timer_sixtop_sendEb_fired(OpenMote* self) {
    ...
    if( openrandom_get16b(self)<(0xffff/EB_PORTION)){
 sixtop_sendEB(self);
    }
}
...
port_INLINE void sixtop_sendEB(OpenMote* self) {
 ...
 packetfunctions_reserveHeaderSize(self, eb,EB_IE_LEN);
    ...
    eb->payload[EB_SLOTFRAME_LEN_OFFSET]   = (uint8_t)(0x00FF & ( schedule_getFrameLength(self)));
    eb->payload[EB_SLOTFRAME_LEN_OFFSET+1] = (uint8_t)(0x00FF & ( schedule_getFrameLength(self)>>8));
...
port_INLINE void sixtop_sendKA(OpenMote* self) {
 ...
 openqueue_removeAllCreatedBy(self, COMPONENT_SIXTOP);
    ...
    if ( schedule_hasManagedTxCellToNeighbor(self, kaNeighAddr) == FALSE){
        // delete packets genereted by this module (EB and KA) from openqueue
 openqueue_removeAllCreatedBy(self, COMPONENT_SIXTOP);
...
void timer_sixtop_six2six_timeout_fired(OpenMote* self) {
 ...
 schedule_removeAllManagedUnicastCellsToNeighbor(self, 
...
void sixtop_six2six_notifyReceive(OpenMote* self, 
    ...
    if (type == SIXTOP_CELL_REQUEST){
        ...
        if ( openqueue_getNum6PResp(self)>=MAX6PRESPONSE) {
            return;
        }
 ...
 openqueue_remove6PrequestToNeighbor(self, &(pkt->l2_nextORpreviousHop));
...
bool sixtop_areAvailableCellsToBeRemoved(OpenMote* self, 
 ...
 schedule_getSlotInfo(self, cellList[i].slotoffset,&info);
                } else {
 schedule_getSlotInfo(self, cellList[i].slotoffset,&info);
                }
