...
This was done on 2022-01-19 11:45:54.533000.
*/
...
open_addr_t* neighbors_getJoinProxy(OpenMote* self) {
 ...
 neighbor_removeAllAutonomousTxRxCellUnicast(self);
...
void neighbor_removeAllAutonomousTxRxCellUnicast(OpenMote* self){
 ...
 schedule_removeAllAutonomousTxRxCellUnicast(self);
}
        ...
        while(i<MAXNUMNEIGHBORS) {
            if ((self->neighbors_vars).neighbors[i].used==FALSE) {
                ...
                if (joinPrioPresent==TRUE){
                    ...
                    (self->neighbors_vars).neighbors[i].joinPrio=DEFAULTJOINPRIORITY;
                }
                break;
            }
            i++;
