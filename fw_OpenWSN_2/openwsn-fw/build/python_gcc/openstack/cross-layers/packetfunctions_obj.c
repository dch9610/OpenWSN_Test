...
This was done on 2022-01-19 11:46:37.824000.
*/
...
void onesComplementSum(OpenMote* self, uint8_t* global_sum, uint8_t* ptr, int length);
...
void packetfunctions_ip128bToMac64b(OpenMote* self, 
      open_addr_t* ip128b,
      open_addr_t* prefix64btoWrite,
      open_addr_t* mac64btoWrite) {
   if (ip128b->type!=ADDR_128B) {
 openserial_printCritical(self, COMPONENT_PACKETFUNCTIONS,ERR_WRONG_ADDR_TYPE,
...
void packetfunctions_mac64bToIp128b(OpenMote* self, 
...
bool packetfunctions_isLinkLocal(OpenMote* self, open_addr_t* address) {
    if (
      address->type          == ADDR_128B &&
...
void packetfunctions_reserveHeaderSize(OpenMote* self, OpenQueueEntry_t* pkt, uint8_t header_length) {
 ...
 openserial_printCritical(self, COMPONENT_PACKETFUNCTIONS,ERR_HEADER_TOO_LONG,
                            (errorparameter_t)0,
...
void packetfunctions_tossHeader(OpenMote* self, OpenQueueEntry_t* pkt, uint8_t header_length) {
 ...
 openserial_printError(self, COMPONENT_PACKETFUNCTIONS,ERR_HEADER_TOO_LONG,
                            (errorparameter_t)1,
                            (errorparameter_t)pkt->length);
   }
}
...
void packetfunctions_calculateChecksum(OpenMote* self, OpenQueueEntry_t* msg, uint8_t* checksum_ptr) {
    ...
    if ( packetfunctions_isBroadcastMulticast(self, &msg->l3_destinationAdd)==TRUE){
        // use link local address for source address (prefix and EUI64)
        
        // source address
 onesComplementSum(self, temp_checksum,(uint8_t*)linklocalprefix,8);
        memcpy(&localscopeAddress, idmanager_getMyID(self, ADDR_64B),sizeof(open_addr_t));
        // invert 'u' bit (section 2.5.1 at https://www.ietf.org/rfc/rfc2373.txt)
        localscopeAddress.addr_64b[0] ^= 0x02;
 onesComplementSum(self, temp_checksum,localscopeAddress.addr_64b,8);
        
        // boardcast destination address
 onesComplementSum(self, temp_checksum,msg->l3_destinationAdd.addr_128b,16);
    } else {
        // use 128-bit ipv6 address for source address and destination address
      
        // source address
 onesComplementSum(self, temp_checksum,( idmanager_getMyID(self, ADDR_PREFIX))->prefix,8);
 onesComplementSum(self, temp_checksum,( idmanager_getMyID(self, ADDR_64B))->addr_64b,8);
        // destination address
 onesComplementSum(self, temp_checksum,msg->l3_destinationAdd.addr_128b,16);
    }
 ...
 onesComplementSum(self, temp_checksum,little_helper,2);
 ...
 onesComplementSum(self, temp_checksum,msg->payload,msg->length);
