/**
DO NOT EDIT DIRECTLY!!

This file was 'objectified' by SCons as a pre-processing
step for the building a Python extension module.

This was done on 2022-01-19 11:47:12.645000.
*/
/**
\brief Definition of the "openserial" driver.

\author Fabien Chraim <chraim@eecs.berkeley.edu>, March 2012.
\author Thomas Watteyne <thomas.watteyne@inria.fr>, August 2016.
*/
#include <stdio.h>
#include <stdarg.h>
#include <string.h>

#include "opendefs_obj.h"
#include "openserial_obj.h"
#include "scheduler_obj.h"
#include "IEEE802154E_obj.h"
#include "neighbors_obj.h"
#include "sixtop_obj.h"
#include "icmpv6echo_obj.h"
#include "idmanager_obj.h"
#include "openqueue_obj.h"
#include "openbridge_obj.h"
#include "leds_obj.h"
#include "schedule_obj.h"
#include "uart_obj.h"
#include "opentimers_obj.h"
#include "openhdlc_obj.h"
#include "schedule_obj.h"
#include "icmpv6rpl_obj.h"
#include "icmpv6echo_obj.h"
#include "msf_obj.h"
#include "debugpins_obj.h"

//=========================== variables =======================================

// declaration of global variable _openserial_vars_ removed during objectification.

enum{
    TYPE_STR = 0,       //subtype for the printf message
    TYPE_INT = 1
};

#define DEBUGPRINT_PERIOD 100 // in ms

//=========================== prototypes ======================================


// printing
owerror_t openserial_printInfoErrorCritical(OpenMote* self, 
    char             severity,
    uint8_t          calling_component,
    uint8_t          error_code,
    errorparameter_t arg1,
    errorparameter_t arg2
);

// command handlers
void openserial_handleRxFrame(OpenMote* self);
void openserial_handleEcho(OpenMote* self, uint8_t* but, uint8_t bufLen);
void openserial_get6pInfo(OpenMote* self, uint8_t commandId, uint8_t* code,uint8_t* cellOptions,uint8_t* numCells,cellInfo_ht* celllist_add,cellInfo_ht* celllist_delete,uint8_t* listOffset,uint8_t* maxListLen,uint8_t ptr, uint8_t commandLen);
void openserial_handleCommands(OpenMote* self);

// misc
void openserial_debugPrint_timer_cb(OpenMote* self, opentimers_id_t id);
void openserial_board_reset_cb(OpenMote* self, opentimers_id_t id);

// HDLC output
void outputHdlcOpen(OpenMote* self);
void outputHdlcWrite(OpenMote* self, uint8_t b);
void outputHdlcClose(OpenMote* self);

// HDLC input
void inputHdlcOpen(OpenMote* self);
void inputHdlcWrite(OpenMote* self, uint8_t b);
void inputHdlcClose(OpenMote* self);

// task
void task_printWrongCRCInput(OpenMote* self);
void task_printInputBufferOverflow(OpenMote* self);

//=========================== public ==========================================

//===== admin

void openserial_init(OpenMote* self) {
    // reset variable
    memset(&(self->openserial_vars),0,sizeof(openserial_vars_t));

    // admin
    (self->openserial_vars).fInhibited         = FALSE;
    (self->openserial_vars).ctsStateChanged    = FALSE;
    (self->openserial_vars).debugPrintCounter  = 0;

    // input
    (self->openserial_vars).hdlcBusyReceiving  = FALSE;
    (self->openserial_vars).hdlcInputEscaping  = FALSE;
    (self->openserial_vars).inputBufFillLevel  = 0;

    // ouput
    (self->openserial_vars).outputBufIdxR      = 0;
    (self->openserial_vars).outputBufIdxW      = 0;
    (self->openserial_vars).fBusyFlushing      = FALSE;

    (self->openserial_vars).reset_timerId      = opentimers_create(self, TIMER_GENERAL_PURPOSE, TASKPRIO_OPENSERIAL);
    (self->openserial_vars).debugPrint_timerId = opentimers_create(self, TIMER_GENERAL_PURPOSE, TASKPRIO_OPENSERIAL);
 opentimers_scheduleIn(self, 
        (self->openserial_vars).debugPrint_timerId,
        DEBUGPRINT_PERIOD,
        TIME_MS,
        TIMER_PERIODIC,
        openserial_debugPrint_timer_cb
    );

    // UART
 uart_setCallbacks(self, 
        isr_openserial_tx,
        isr_openserial_rx
    );
 uart_enableInterrupts(self);
}

void openserial_register(OpenMote* self, openserial_rsvpt* rsvp) {
    // FIXME: register multiple commands (linked list)
    (self->openserial_vars).registeredCmd = rsvp;
}

//===== transmitting

owerror_t openserial_printStatus(OpenMote* self, 
    uint8_t             statusElement,
    uint8_t*            buffer,
    uint8_t             length
) {
    uint8_t i;

 outputHdlcOpen(self);
 outputHdlcWrite(self, SERFRAME_MOTE2PC_STATUS);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[0]);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[1]);
 outputHdlcWrite(self, statusElement);
    for (i=0;i<length;i++){
 outputHdlcWrite(self, buffer[i]);
    }
 outputHdlcClose(self);

    // start TX'ing
 openserial_flush(self);

    return E_SUCCESS;
}

owerror_t openserial_printInfo(OpenMote* self, 
    uint8_t             calling_component,
    uint8_t             error_code,
    errorparameter_t    arg1,
    errorparameter_t    arg2
) {
    return openserial_printInfoErrorCritical(self, 
        SERFRAME_MOTE2PC_INFO,
        calling_component,
        error_code,
        arg1,
        arg2
    );

    // openserial_flush called by openserial_printInfoErrorCritical(self)
}

owerror_t openserial_printError(OpenMote* self, 
    uint8_t             calling_component,
    uint8_t             error_code,
    errorparameter_t    arg1,
    errorparameter_t    arg2
) {
    // toggle error LED
 leds_error_toggle(self);

    return openserial_printInfoErrorCritical(self, 
        SERFRAME_MOTE2PC_ERROR,
        calling_component,
        error_code,
        arg1,
        arg2
    );

    // openserial_flush called by openserial_printInfoErrorCritical(self)
}

owerror_t openserial_printCritical(OpenMote* self, 
    uint8_t             calling_component,
    uint8_t             error_code,
    errorparameter_t    arg1,
    errorparameter_t    arg2
) {
    uint32_t         reference;

    // blink error LED, this is serious
 leds_error_blink(self);

    // schedule for the mote to reboot in 10s
    reference = opentimers_getValue(self);
 opentimers_scheduleAbsolute(self, 
        (self->openserial_vars).reset_timerId,  // timerId
        10000,                          // duration
        reference,                      // reference
        TIME_MS,                        // timetype
        openserial_board_reset_cb       // callback
    );

    return openserial_printInfoErrorCritical(self, 
        SERFRAME_MOTE2PC_CRITICAL,
        calling_component,
        error_code,
        arg1,
        arg2
    );

    // openserial_flush called by openserial_printInfoErrorCritical(self)
}

owerror_t openserial_printData(OpenMote* self, uint8_t* buffer, uint8_t length) {
    uint8_t  i;
    uint8_t  asn[5];

    // retrieve ASN
 ieee154e_getAsn(self, asn);

 outputHdlcOpen(self);
 outputHdlcWrite(self, SERFRAME_MOTE2PC_DATA);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[0]);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[1]);
 outputHdlcWrite(self, asn[0]);
 outputHdlcWrite(self, asn[1]);
 outputHdlcWrite(self, asn[2]);
 outputHdlcWrite(self, asn[3]);
 outputHdlcWrite(self, asn[4]);
    for (i=0;i<length;i++){
 outputHdlcWrite(self, buffer[i]);
    }
 outputHdlcClose(self);

    // start TX'ing
 openserial_flush(self);

    return E_SUCCESS;
}

owerror_t openserial_printSniffedPacket(OpenMote* self, uint8_t* buffer, uint8_t length, uint8_t channel) {
    uint8_t  i;

 outputHdlcOpen(self);
 outputHdlcWrite(self, SERFRAME_MOTE2PC_SNIFFED_PACKET);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[0]);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[1]);
    for (i=0;i<length;i++){
 outputHdlcWrite(self, buffer[i]);
    }
 outputHdlcWrite(self, channel);
 outputHdlcClose(self);

    // start TX'ing
 openserial_flush(self);

    return E_SUCCESS;
}

owerror_t openserial_print_uint32_t(OpenMote* self, uint32_t value) {
#ifdef OPENSERIAL_PRINTF
    uint8_t  i;
    uint8_t  pvalue[4];

 outputHdlcOpen(self);
 outputHdlcWrite(self, SERFRAME_MOTE2PC_PRINTF);
 outputHdlcWrite(self, TYPE_INT);
    memcpy(pvalue, &value, 4);
    for (i=0;i<4;i++) {
 outputHdlcWrite(self, pvalue[i]);
    }
 outputHdlcClose(self);

    // start TX'ing
 openserial_flush(self);
#endif
    return E_SUCCESS;
}

owerror_t openserial_print_str(OpenMote* self, char* buffer, uint8_t length) {
#ifdef OPENSERIAL_PRINTF
    uint8_t  i;
    uint8_t  asn[5];

    // retrieve ASN
 ieee154e_getAsn(self, asn);

 outputHdlcOpen(self);
 outputHdlcWrite(self, SERFRAME_MOTE2PC_PRINTF);
 outputHdlcWrite(self, TYPE_STR);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[0]);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[1]);
    for(i=0; i<5;i++) {
 outputHdlcWrite(self, asn[i]);
    }
    for (i=0;i<length;i++){
 outputHdlcWrite(self, buffer[i]);
    }
 outputHdlcClose(self);

    // start TX'ing
 openserial_flush(self);
#endif
   return E_SUCCESS;
}

//===== retrieving inputBuffer

void task_openserial_debugPrint(OpenMote* self) {
    uint8_t debugPrintCounter;
    INTERRUPT_DECLARATION();

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();
    debugPrintCounter = (self->openserial_vars).debugPrintCounter;
    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>

    if ((self->openserial_vars).outputBufIdxW!=(self->openserial_vars).outputBufIdxR) {
        return;
    }

    debugPrintCounter++;
    if (debugPrintCounter==STATUS_MAX) {
       debugPrintCounter = 0;
    }

    switch (debugPrintCounter) {
        case STATUS_ISSYNC:
            if ( debugPrint_isSync(self)==TRUE) {
                break;
            }
        case STATUS_ID:
            if ( debugPrint_id(self)==TRUE) {
               break;
            }
        case STATUS_DAGRANK:
            if ( debugPrint_myDAGrank(self)==TRUE) {
                break;
            }
        case STATUS_OUTBUFFERINDEXES:
            if ( debugPrint_outBufferIndexes(self)==TRUE) {
                break;
            }
        case STATUS_ASN:
            if ( debugPrint_asn(self)==TRUE) {
                break;
            }
        case STATUS_MACSTATS:
            if ( debugPrint_macStats(self)==TRUE) {
                break;
            }
        case STATUS_SCHEDULE:
            if( debugPrint_schedule(self)==TRUE) {
                break;
            }
        case STATUS_BACKOFF:
            if( debugPrint_backoff(self)==TRUE) {
                break;
            }
        case STATUS_QUEUE:
            if( debugPrint_queue(self)==TRUE) {
                break;
            }
        case STATUS_NEIGHBORS:
            if ( debugPrint_neighbors(self)==TRUE) {
                break;
            }
        case STATUS_KAPERIOD:
            if ( debugPrint_kaPeriod(self)==TRUE) {
                break;
            }
        case STATUS_JOINED:
            if ( debugPrint_joined(self)==TRUE) {
                break;
            }
        default:
            debugPrintCounter=0;
    }

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();
    (self->openserial_vars).debugPrintCounter = debugPrintCounter;
    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>
}

//===== receiving

uint8_t openserial_getInputBufferFillLevel(OpenMote* self) {
    uint8_t inputBufFillLevel;
    INTERRUPT_DECLARATION();

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();
    inputBufFillLevel = (self->openserial_vars).inputBufFillLevel;
    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>

    return inputBufFillLevel-1; // removing the command byte
}

uint8_t openserial_getInputBuffer(OpenMote* self, uint8_t* bufferToWrite, uint8_t maxNumBytes) {
    uint8_t numBytesWritten;
    uint8_t inputBufFillLevel;
    INTERRUPT_DECLARATION();

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();
    inputBufFillLevel = (self->openserial_vars).inputBufFillLevel;
    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>

    if (maxNumBytes<inputBufFillLevel-1) {
 openserial_printError(self, 
            COMPONENT_OPENSERIAL,
            ERR_GETDATA_ASKS_TOO_FEW_BYTES,
            (errorparameter_t)maxNumBytes,
            (errorparameter_t)inputBufFillLevel-1
        );
        numBytesWritten = 0;
    } else {
        numBytesWritten = inputBufFillLevel-1;
        //<<<<<<<<<<<<<<<<<<<<<<<
        DISABLE_INTERRUPTS();
        memcpy(bufferToWrite,&((self->openserial_vars).inputBuf[1]),numBytesWritten);
        ENABLE_INTERRUPTS();
        //>>>>>>>>>>>>>>>>>>>>>>>
    }

    return numBytesWritten;
}

//===== scheduling

void openserial_flush(OpenMote* self) {
    INTERRUPT_DECLARATION();

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();
    if ((self->openserial_vars).fBusyFlushing==FALSE) {
        if ((self->openserial_vars).ctsStateChanged==TRUE) {
            // send CTS
#ifdef FASTSIM
#else
            if ((self->openserial_vars).fInhibited==TRUE) {
 uart_setCTS(self, FALSE);
            } else {
 uart_setCTS(self, TRUE);
            }
#endif
            (self->openserial_vars).ctsStateChanged = FALSE;
        } else {
            if ((self->openserial_vars).fInhibited==TRUE) {
                // currently inhibited
            } else {
                // not inhibited
                if ((self->openserial_vars).outputBufIdxW!=(self->openserial_vars).outputBufIdxR) {
                    // I have some bytes to transmit

#ifdef FASTSIM
 uart_writeCircularBuffer_FASTSIM(self, 
                        (self->openserial_vars).outputBuf,
                        &(self->openserial_vars).outputBufIdxR,
                        &(self->openserial_vars).outputBufIdxW
                    );
#else
 uart_writeByte(self, (self->openserial_vars).outputBuf[OUTPUT_BUFFER_MASK & ((self->openserial_vars).outputBufIdxR++)]);
                    (self->openserial_vars).fBusyFlushing = TRUE;
#endif
                }
            }
        }
    }
    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>
}

void openserial_inhibitStart(OpenMote* self) {
    // this function needs to run in non-interrupt mode
    // since the inhibitStart is always called in an interrupt mode,
    // DISABLE_INTERRUPT is not necessary here.

    (self->openserial_vars).fInhibited      = TRUE;
#ifdef FASTSIM
#else
    (self->openserial_vars).ctsStateChanged = TRUE;
#endif

    // it's openserial_flush(self) which will set CTS
 openserial_flush(self);
}

void openserial_inhibitStop(OpenMote* self) {
    INTERRUPT_DECLARATION();

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();
    (self->openserial_vars).fInhibited      = FALSE;
#ifdef FASTSIM
#else
    (self->openserial_vars).ctsStateChanged = TRUE;
#endif
    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>

    // it's openserial_flush(self) which will set CTS
 openserial_flush(self);
}

//===== debugprint

/**
\brief Trigger this module to print status information, over serial.

debugPrint_* functions are used by the openserial module to continuously print
status information about several modules in the OpenWSN stack.

\returns TRUE if this function printed something, FALSE otherwise.
*/
bool debugPrint_outBufferIndexes(OpenMote* self) {
    uint16_t temp_buffer[2];
    INTERRUPT_DECLARATION();

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();
    temp_buffer[0] = (self->openserial_vars).outputBufIdxW;
    temp_buffer[1] = (self->openserial_vars).outputBufIdxR;
    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>

 openserial_printStatus(self, 
        STATUS_OUTBUFFERINDEXES,
        (uint8_t*)temp_buffer,
        sizeof(temp_buffer)
    );

    return TRUE;
}

//=========================== private =========================================

//===== printing

owerror_t openserial_printInfoErrorCritical(OpenMote* self, 
    char             severity,
    uint8_t          calling_component,
    uint8_t          error_code,
    errorparameter_t arg1,
    errorparameter_t arg2
) {

 outputHdlcOpen(self);
 outputHdlcWrite(self, severity);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[0]);
 outputHdlcWrite(self, idmanager_getMyID(self, ADDR_16B)->addr_16b[1]);
 outputHdlcWrite(self, calling_component);
 outputHdlcWrite(self, error_code);
 outputHdlcWrite(self, (uint8_t)((arg1 & 0xff00)>>8));
 outputHdlcWrite(self, (uint8_t) (arg1 & 0x00ff));
 outputHdlcWrite(self, (uint8_t)((arg2 & 0xff00)>>8));
 outputHdlcWrite(self, (uint8_t) (arg2 & 0x00ff));
 outputHdlcClose(self);

    // start TX'ing
 openserial_flush(self);

    return E_SUCCESS;
}

//===== command handlers

// executed in ISR
void openserial_handleRxFrame(OpenMote* self) {
    uint8_t cmdByte;

    cmdByte = (self->openserial_vars).inputBuf[0];
    // call hard-coded commands
    // FIXME: needs to be replaced by registered commands only
    switch (cmdByte) {
        case SERFRAME_PC2MOTE_SETROOT:
 idmanager_triggerAboutRoot(self);
            break;
        case SERFRAME_PC2MOTE_RESET:
 board_reset(self);
            break;
        case SERFRAME_PC2MOTE_DATA:
 openbridge_triggerData(self);
            break;
        case SERFRAME_PC2MOTE_TRIGGERSERIALECHO:
 openserial_handleEcho(self, 
                &(self->openserial_vars).inputBuf[1],
                (self->openserial_vars).inputBufFillLevel-1
            );
            break;
        case SERFRAME_PC2MOTE_COMMAND:
 openserial_handleCommands(self);
            break;
    }
    // call registered commands
    if ((self->openserial_vars).registeredCmd!=NULL && (self->openserial_vars).registeredCmd->cmdId==cmdByte) {
        (self->openserial_vars).registeredCmd->cb(self);
    }
}

void openserial_handleEcho(OpenMote* self, uint8_t* buf, uint8_t bufLen){
    // echo back what you received
 openserial_printData(self, 
        buf,
        bufLen
    );
}

void openserial_get6pInfo(OpenMote* self, uint8_t commandId, uint8_t* code,uint8_t* cellOptions,uint8_t* numCells,cellInfo_ht* celllist_add,cellInfo_ht* celllist_delete,uint8_t* listOffset,uint8_t* maxListLen,uint8_t ptr, uint8_t commandLen){
    uint8_t i;
    uint8_t celllistLen;

    // clear command
    if (commandId == COMMAND_SET_6P_CLEAR){
        *code = IANA_6TOP_CMD_CLEAR;
        return;
    }

    *cellOptions  = (self->openserial_vars).inputBuf[ptr];
    ptr          += 1;
    commandLen   -= 1;

    // list command
    if (commandId == COMMAND_SET_6P_LIST){
        *code = IANA_6TOP_CMD_LIST;
        *listOffset   = (self->openserial_vars).inputBuf[ptr];
        ptr += 1;
        *maxListLen   = (self->openserial_vars).inputBuf[ptr];
        ptr += 1;
        return;
    }

    // count command
    if (commandId == COMMAND_SET_6P_COUNT){
        *code = IANA_6TOP_CMD_COUNT;
        return;
    }

    *numCells   = (self->openserial_vars).inputBuf[ptr];
    ptr        += 1;
    commandLen -= 1;

    // add command
    if (commandId == COMMAND_SET_6P_ADD){
        *code = IANA_6TOP_CMD_ADD;
        // retrieve cell list
        i = 0;
        celllistLen = commandLen/2;
        while(commandLen>celllistLen){
            celllist_add[i].slotoffset     = (self->openserial_vars).inputBuf[ptr];
            celllist_add[i].channeloffset  = (self->openserial_vars).inputBuf[ptr+celllistLen];
            celllist_add[i].isUsed         = TRUE;
            ptr         += 1;
            commandLen  -= 1;
            i++;
        }
        return;
    }

    // delete command
    if (commandId == COMMAND_SET_6P_DELETE){
        *code = IANA_6TOP_CMD_DELETE;
        // retrieve cell list
        i = 0;
        celllistLen = commandLen/2;
        while(commandLen>celllistLen){
            celllist_delete[i].slotoffset     = (self->openserial_vars).inputBuf[ptr];
            celllist_delete[i].channeloffset  = (self->openserial_vars).inputBuf[ptr+celllistLen];
            celllist_delete[i].isUsed         = TRUE;
            ptr         += 1;
            commandLen  -= 1;
            i++;
        }
        return;
    }

    // relocate command
    if (commandId == COMMAND_SET_6P_RELOCATE){
        *code = IANA_6TOP_CMD_RELOCATE;
        // retrieve cell list to be relocated
        i = 0;
        while(i<*numCells){
            celllist_delete[i].slotoffset     = (self->openserial_vars).inputBuf[ptr];
            celllist_delete[i].channeloffset  = (self->openserial_vars).inputBuf[ptr+*numCells];
            celllist_delete[i].isUsed         = TRUE;
            ptr         += 1;
            i++;
        }

        commandLen      -= (*numCells) * 2;
        ptr             += *numCells;
        // retrieve cell list to be relocated
        i = 0;
        celllistLen = commandLen/2;
        while(commandLen>celllistLen){
            celllist_add[i].slotoffset     = (self->openserial_vars).inputBuf[ptr];
            celllist_add[i].channeloffset  = (self->openserial_vars).inputBuf[ptr+celllistLen];
            celllist_add[i].isUsed         = TRUE;
            ptr         += 1;
            commandLen  -= 1;
            i++;
        }
        return;
    }
}

void openserial_handleCommands(OpenMote* self){
    uint8_t  input_buffer[20];
    uint8_t  numDataBytes;
    uint8_t  commandId;
    uint8_t  commandLen;
    uint8_t  comandParam_8;
    uint16_t comandParam_16;

    uint8_t  code,cellOptions,numCell,listOffset,maxListLen;
    uint8_t  ptr;
    cellInfo_ht celllist_add[CELLLIST_MAX_LEN];
    cellInfo_ht celllist_delete[CELLLIST_MAX_LEN];

    open_addr_t neighbor;
    bool        foundNeighbor;

    ptr = 0;
    memset(celllist_add,0,CELLLIST_MAX_LEN*sizeof(cellInfo_ht));
    memset(celllist_delete,0,CELLLIST_MAX_LEN*sizeof(cellInfo_ht));

   numDataBytes = openserial_getInputBufferFillLevel(self);
   //copying the buffer
 openserial_getInputBuffer(self, &(input_buffer[ptr]),numDataBytes);
    ptr++;
    commandId  = (self->openserial_vars).inputBuf[ptr];
    ptr++;
    commandLen = (self->openserial_vars).inputBuf[ptr];
    ptr++;

    switch(commandId) {
        case COMMAND_SET_EBPERIOD:
            comandParam_8 = (self->openserial_vars).inputBuf[ptr];
 sixtop_setEBPeriod(self, comandParam_8); // one byte, in seconds
            break;
        case COMMAND_SET_CHANNEL:
            comandParam_8 = (self->openserial_vars).inputBuf[ptr];
            // set communication channel for protocol stack
 ieee154e_setSingleChannel(self, comandParam_8); // one byte
            break;
        case COMMAND_SET_KAPERIOD: // two bytes, in slots
            comandParam_16 = ((self->openserial_vars).inputBuf[ptr] & 0x00ff) | \
                (((self->openserial_vars).inputBuf[ptr+1]<<8) & 0xff00);
 sixtop_setKaPeriod(self, comandParam_16);
            break;
        case COMMAND_SET_DIOPERIOD: // two bytes, in mili-seconds
            comandParam_16 = ((self->openserial_vars).inputBuf[ptr] & 0x00ff) | \
                (((self->openserial_vars).inputBuf[ptr+1]<<8) & 0xff00);
 icmpv6rpl_setDIOPeriod(self, comandParam_16);
            break;
        case COMMAND_SET_DAOPERIOD: // two bytes, in mili-seconds
            comandParam_16 = ((self->openserial_vars).inputBuf[ptr] & 0x00ff) | \
                (((self->openserial_vars).inputBuf[ptr+1]<<8) & 0xff00);
 icmpv6rpl_setDAOPeriod(self, comandParam_16);
            break;
        case COMMAND_SET_DAGRANK: // two bytes
            comandParam_16 = ((self->openserial_vars).inputBuf[ptr] & 0x00ff) | \
                (((self->openserial_vars).inputBuf[ptr+1]<<8) & 0xff00);
 icmpv6rpl_setMyDAGrank(self, comandParam_16);
            break;
        case COMMAND_SET_SECURITY_STATUS: // one byte
            comandParam_8 = (self->openserial_vars).inputBuf[ptr];
 ieee154e_setIsSecurityEnabled(self, comandParam_8);
                   break;
        case COMMAND_SET_SLOTFRAMELENGTH: // two bytes
            comandParam_16 = ((self->openserial_vars).inputBuf[ptr] & 0x00ff) | \
                (((self->openserial_vars).inputBuf[ptr+1]<<8) & 0xff00);
 schedule_setFrameLength(self, comandParam_16);
            break;
        case COMMAND_SET_ACK_STATUS:
            comandParam_8 = (self->openserial_vars).inputBuf[ptr];
 ieee154e_setIsAckEnabled(self, comandParam_8);
                   break;
        case COMMAND_SET_6P_ADD:
        case COMMAND_SET_6P_DELETE:
        case COMMAND_SET_6P_RELOCATE:
        case COMMAND_SET_6P_COUNT:
        case COMMAND_SET_6P_LIST:
        case COMMAND_SET_6P_CLEAR:
            // get preferred parent
            foundNeighbor = icmpv6rpl_getPreferredParentEui64(self, &neighbor);
            if (foundNeighbor==FALSE) {
                break;
            }
            // the following sequence of bytes are, slotframe, cellOption, numCell, celllist
 openserial_get6pInfo(self, commandId,&code,&cellOptions,&numCell,celllist_add,celllist_delete,&listOffset,&maxListLen,ptr,commandLen);
 sixtop_request(self, 
                code,              // code
                &neighbor,         // neighbor
                numCell,           // number cells
                cellOptions,       // cellOptions
                celllist_add,      // celllist to add
                celllist_delete,   // celllist to delete (not used)
 msf_getsfid(self),     // sfid
                listOffset,        // list command offset (not used)
                maxListLen         // list command maximum celllist (not used)
            );
            break;
        case COMMAND_SET_SLOTDURATION:
            comandParam_16 = ((self->openserial_vars).inputBuf[ptr] & 0x00ff) | \
                (((self->openserial_vars).inputBuf[ptr+1]<<8) & 0xff00);
 ieee154e_setSlotDuration(self, comandParam_16);
            break;
        case COMMAND_SET_6PRESPONSE:
            comandParam_8 = (self->openserial_vars).inputBuf[ptr];
            if (comandParam_8 ==1) {
 sixtop_setIsResponseEnabled(self, TRUE);
            } else {
                if (comandParam_8 == 0) {
 sixtop_setIsResponseEnabled(self, FALSE);
                } else {
                    // security only can be 1 or 0
                    break;
                }
            }
            break;
        case COMMAND_SET_UINJECTPERIOD:
            comandParam_8 = (self->openserial_vars).inputBuf[ptr];
 msf_appPktPeriod(self, comandParam_8);
            break;
        case COMMAND_SET_ECHO_REPLY_STATUS:
            comandParam_8 = (self->openserial_vars).inputBuf[ptr];
            if (comandParam_8 == 1) {
 icmpv6echo_setIsReplyEnabled(self, TRUE);
            } else {
                if (comandParam_8 == 0) {
 icmpv6echo_setIsReplyEnabled(self, FALSE);
                } else {
                    // ack reply
                    break;
                }
            }
            break;
        case COMMAND_SET_JOIN_KEY:
            if (commandLen != 16) { break; }
 idmanager_setJoinKey(self, &(self->openserial_vars).inputBuf[ptr]);
            break;
        default:
            // wrong command ID
            break;
   }
}

//===== misc

void openserial_debugPrint_timer_cb(OpenMote* self, opentimers_id_t id){
    // calling the task directly as the timer_cb function is executed in
    // task mode by opentimer already
 task_openserial_debugPrint(self);
}

void openserial_board_reset_cb(OpenMote* self, opentimers_id_t id) {
    (void)id;
 board_reset(self);
}

//===== hdlc (output)

/**
\brief Start an HDLC frame in the output buffer.
*/
port_INLINE void outputHdlcOpen(OpenMote* self) {
    INTERRUPT_DECLARATION();

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();

    // initialize the value of the CRC

    (self->openserial_vars).hdlcOutputCrc                                    = HDLC_CRCINIT;

    // write the opening HDLC flag
    (self->openserial_vars).outputBuf[OUTPUT_BUFFER_MASK & ((self->openserial_vars).outputBufIdxW++)]       = HDLC_FLAG;

    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>
}
/**
\brief Add a byte to the outgoing HDLC frame being built.
*/
port_INLINE void outputHdlcWrite(OpenMote* self, uint8_t b) {
    INTERRUPT_DECLARATION();

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();

    // iterate through CRC calculator
    (self->openserial_vars).hdlcOutputCrc = crcIteration((self->openserial_vars).hdlcOutputCrc,b);

    // add byte to buffer
    if (b==HDLC_FLAG || b==HDLC_ESCAPE) {
        (self->openserial_vars).outputBuf[OUTPUT_BUFFER_MASK & ((self->openserial_vars).outputBufIdxW++)]   = HDLC_ESCAPE;
        b                                                            = b^HDLC_ESCAPE_MASK;
    }
    (self->openserial_vars).outputBuf[OUTPUT_BUFFER_MASK & ((self->openserial_vars).outputBufIdxW++)]       = b;

    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>
}
/**
\brief Finalize the outgoing HDLC frame.
*/
port_INLINE void outputHdlcClose(OpenMote* self) {
    uint16_t   finalCrc;
    INTERRUPT_DECLARATION();

    //<<<<<<<<<<<<<<<<<<<<<<<
    DISABLE_INTERRUPTS();

    // finalize the calculation of the CRC
    finalCrc   = ~(self->openserial_vars).hdlcOutputCrc;

    // write the CRC value
 outputHdlcWrite(self, (finalCrc>>0)&0xff);
 outputHdlcWrite(self, (finalCrc>>8)&0xff);

    // write the closing HDLC flag
    (self->openserial_vars).outputBuf[OUTPUT_BUFFER_MASK & ((self->openserial_vars).outputBufIdxW++)]       = HDLC_FLAG;

    ENABLE_INTERRUPTS();
    //>>>>>>>>>>>>>>>>>>>>>>>
}

//===== hdlc (input)

/**
\brief Start an HDLC frame in the input buffer.
*/
port_INLINE void inputHdlcOpen(OpenMote* self) {
    // reset the input buffer index
    (self->openserial_vars).inputBufFillLevel                                = 0;

    // initialize the value of the CRC
    (self->openserial_vars).hdlcInputCrc                                     = HDLC_CRCINIT;
}
/**
\brief Add a byte to the incoming HDLC frame.
*/
port_INLINE void inputHdlcWrite(OpenMote* self, uint8_t b) {
    if (b==HDLC_ESCAPE) {
        (self->openserial_vars).hdlcInputEscaping = TRUE;
    } else {
        if ((self->openserial_vars).hdlcInputEscaping==TRUE) {
            b                             = b^HDLC_ESCAPE_MASK;
            (self->openserial_vars).hdlcInputEscaping = FALSE;
        }

        // add byte to input buffer
        (self->openserial_vars).inputBuf[(self->openserial_vars).inputBufFillLevel] = b;
        (self->openserial_vars).inputBufFillLevel++;

        // iterate through CRC calculator
        (self->openserial_vars).hdlcInputCrc = crcIteration((self->openserial_vars).hdlcInputCrc,b);
    }
}
/**
\brief Finalize the incoming HDLC frame.
*/
port_INLINE void inputHdlcClose(OpenMote* self) {

    // verify the validity of the frame
    if ((self->openserial_vars).hdlcInputCrc==HDLC_CRCGOOD) {
        // the CRC is correct

        // remove the CRC from the input buffer
        (self->openserial_vars).inputBufFillLevel    -= 2;
    } else {
        // the CRC is incorrect

        // drop the incoming frame
        (self->openserial_vars).inputBufFillLevel     = 0;
    }
}

//=========================== task ============================================

void task_printInputBufferOverflow(OpenMote* self){
    // input buffer overflow
 openserial_printError(self, 
        COMPONENT_OPENSERIAL,
        ERR_INPUT_BUFFER_OVERFLOW,
        (errorparameter_t)0,
        (errorparameter_t)0
    );
}

void task_printWrongCRCInput(OpenMote* self){
    // invalid HDLC frame
 openserial_printError(self, 
        COMPONENT_OPENSERIAL,
        ERR_WRONG_CRC_INPUT,
        (errorparameter_t)0,
        (errorparameter_t)0
    );
}

//=========================== interrupt handlers ==============================

// executed in ISR, called from scheduler.c
void isr_openserial_tx(OpenMote* self) {
    if ((self->openserial_vars).ctsStateChanged==TRUE) {
        // set CTS

        if ((self->openserial_vars).fInhibited==TRUE) {
 uart_setCTS(self, FALSE);
            (self->openserial_vars).fBusyFlushing = FALSE;
        } else {
 uart_setCTS(self, TRUE);
        }
        (self->openserial_vars).ctsStateChanged = FALSE;
    } else if ((self->openserial_vars).fInhibited==TRUE) {
        // currently inhibited

        (self->openserial_vars).fBusyFlushing = FALSE;
    } else {
        // not inhibited

        if ((self->openserial_vars).outputBufIdxW!=(self->openserial_vars).outputBufIdxR) {
            // I have some bytes to transmit

 uart_writeByte(self, (self->openserial_vars).outputBuf[OUTPUT_BUFFER_MASK & ((self->openserial_vars).outputBufIdxR++)]);
            (self->openserial_vars).fBusyFlushing = TRUE;
        } else {
            // I'm done sending bytes

            (self->openserial_vars).fBusyFlushing = FALSE;
        }
    }
}

/**
\pre executed in ISR, called from scheduler.c

\returns 1 if don't receiving frame, 0 if not
*/

uint8_t isr_openserial_rx(OpenMote* self) {
    uint8_t rxbyte;
    uint8_t returnVal;

    returnVal = 0;

    // read byte just received
    rxbyte = uart_readByte(self);

    if (
        (self->openserial_vars).hdlcBusyReceiving==FALSE  &&
        (self->openserial_vars).hdlcLastRxByte==HDLC_FLAG &&
        rxbyte!=HDLC_FLAG
    ) {
        // start of frame

        // I'm now receiving
        (self->openserial_vars).hdlcBusyReceiving         = TRUE;

        // create the HDLC frame
 inputHdlcOpen(self);

        // add the byte just received
 inputHdlcWrite(self, rxbyte);
    } else if (
        (self->openserial_vars).hdlcBusyReceiving==TRUE   &&
        rxbyte!=HDLC_FLAG
    ) {
        // middle of frame

        // add the byte just received
 inputHdlcWrite(self, rxbyte);
        if ((self->openserial_vars).inputBufFillLevel+1>SERIAL_INPUT_BUFFER_SIZE){
            // push task
 scheduler_push_task(self, task_printInputBufferOverflow,TASKPRIO_OPENSERIAL);
            (self->openserial_vars).inputBufFillLevel      = 0;
            (self->openserial_vars).hdlcBusyReceiving      = FALSE;
        }
    } else if (
        (self->openserial_vars).hdlcBusyReceiving==TRUE   &&
        rxbyte==HDLC_FLAG
    ) {
        // end of frame

        // finalize the HDLC frame
 inputHdlcClose(self);
        (self->openserial_vars).hdlcBusyReceiving      = FALSE;

        if ((self->openserial_vars).inputBufFillLevel==0){
            // push task
 scheduler_push_task(self, task_printWrongCRCInput,TASKPRIO_OPENSERIAL);
        } else {
 openserial_handleRxFrame(self);
            (self->openserial_vars).inputBufFillLevel = 0;
            returnVal = 1;
        }
    }

    (self->openserial_vars).hdlcLastRxByte = rxbyte;

    return returnVal;
}



